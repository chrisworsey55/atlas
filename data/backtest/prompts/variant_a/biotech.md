...
## Autoresearch Addition
## AUTOMATIC TRADE REJECTION SYSTEM
**BEFORE ANY RECOMMENDATION, RUN THIS MANDATORY CHECK:**

```
IF (ticker in recent_losers AND days_since_loss < 3):
   RETURN "TRADE REJECTED: Cooling-off period active"
IF (stock_multiple > 15x sales AND (VIX > 16 OR XLV down > 1% OR SPY down > 0.5%)):
   RETURN "TRADE REJECTED: High-multiple block active"
IF (conviction > 40% AND risk_off_conditions):
   RETURN "TRADE REJECTED: Conviction exceeds risk-off limits"
```

**ANY ATTEMPT TO OVERRIDE THESE CHECKS RESULTS IN NO RECOMMENDATION.**
**STATE: "REJECTION CHECK: [PASSED/FAILED - REASON]" BEFORE EVERY TRADE.**
...
class Janus:
    ...
    def __init__(self, cohorts: List[str] = None):
        ...
        self.judge_file = STATE_DIR / "judge_daily.json"
    ...
    def load_judge_output(self) -> Optional[Dict]:
        """
        Load the daily judge output that now acts as JANUS's primary input.

        Returns:
            Judge payload or None if not available.
        """
        if not self.judge_file.exists():
            return None

        try:
            with open(self.judge_file, "r") as f:
                data = json.load(f)
            return data if isinstance(data, dict) else None
        except Exception as e:
            logger.error(f"[janus] Error loading judge output: {e}")
            return None
    ...
    def update_weights(self, outcomes: List[Dict] = None, judge_payload: Dict | None = None):
        """
        ...
        Args:
            outcomes: Optional list of scored outcomes. If not provided,
                     loads from file.
            judge_payload: Optional judge output. When provided, this is the
                           primary input and replaces raw cohort-accuracy
                           scoring as the weighting source.
        """
        if judge_payload is not None:
            raw_scores = self._scores_from_judge(judge_payload)
            if not raw_scores:
                logger.warning("[janus] Judge payload missing cohort metrics; falling back to scored outcomes")
                outcomes = outcomes or self.load_scored_outcomes()
            else:
                self.cohort_weights = self._softmax_with_constraints(raw_scores)
                logger.info(f"[janus] Updated weights from judge input: {self.cohort_weights}")
                logger.info(f"[janus] Judge payload: {judge_payload}")
                return
    ...
    def _scores_from_judge(self, judge_payload: Dict) -> Dict[str, float]:
        """
        Convert judge output into JANUS weighting scores.

        The judge output is the primary input; it should contain a `cohorts`
        mapping with hit_rate and sharpe for each cohort. Missing fields fall
        back to neutral defaults.
        """
        cohorts = judge_payload.get("cohorts") if isinstance(judge_payload, dict) else None
        ...
        for cohort in self.cohorts:
            ...
            self.cohort_accuracy[cohort] = {
                "hit_rate": hit_rate,
                "sharpe": sharpe,
                "judge_confidence": float(metrics.get("judge_confidence", 0.5) or 0.5),
            }
            norm_sharpe = max(0, min(1, (sharpe + 1) / 2))
            raw_scores[cohort] = 0.5 * hit_rate + 0.5 * norm_sharpe
    ...
    def run_daily(self, judge_payload: Dict | None = None) -> Dict:
        """
        Execute full daily JANUS cycle:
        1. Load judge output or scored outcomes
        ...
        judge_input = judge_payload if judge_payload is not None else self.load_judge_output()
        input_source = "judge_daily.json" if judge_input is not None else "scored_outcomes.json"

        # Load and score
        if judge_input is not None:
            self.update_weights(judge_payload=judge_input)
        else:
            outcomes = self.load_scored_outcomes()
